...
`,
      ...
      execute: async ({ code }, { toolCallId }) => {
        ...
        const localSandboxDir = process.env.OPEN_AGENT_LOCAL_SANDBOX_DIR;
        if (!key && localSandboxDir) {
          try {
            return await runLocalSandbox(
              localSandboxDir,
              code,
              toolCallId,
              writer
...
async function runLocalSandbox(
  ...
  code: string,
  toolCallId: string,
  writer: WritableStreamDefaultWriter<StreamObjectToolResult>
) {
  const id = randomUUID();
  const requestDir = join(spoolDir, 'requests');
  const resultDir = join(spoolDir, 'results');
  const pendingPath = join(requestDir, `${id}.tmp`);
  ...
  await writeFile(pendingPath, JSON.stringify({ id, code }), {
    encoding: 'utf8',
    mode: 0o600,
  });
    ...
    try {
      ...
      if (result.stderr) {
        await writer.write({
          type: 'tool-incomplete-result',
          toolCallId,
          data: { type: 'text-delta', textDelta: result.stderr },
        });
      }
      ...
      return {
        error:
          result.exitCode === 0 && !result.timedOut
            ? null
            : {
                name: result.timedOut ? 'TimeoutError' : 'PythonError',
                value: result.stderr || `Process exited ${result.exitCode}`,
              },
        logs: {
          stdout: result.stdout ? [{ line: result.stdout }] : [],
          stderr: result.stderr ? [{ line: result.stderr }] : [],
        },
        result: result.stdout ? [{ text: result.stdout }] : [],
      };
    } catch (error) {
  ...
  return toolError(
    'Local Python Sandbox Failed',
    'The isolated worker did not return within 35 seconds.'
  );
}
