...
export class OpenAIProvider extends CopilotProvider<OpenAIConfig> {
  ...
  readonly models = [
    ...
    // Embedding models
    {
      ...
      capabilities: [{ input: [ModelInputType.Text], output: [ModelOutputType.Embedding] }],
    },
    {
      id: 'mistral-embed',
      capabilities: [
  ...
  override async embedding(
    ...
    const embeddingProvider = this.config.embedding
      ? createOpenAICompatible({ name: 'local-embedding', ...this.config.embedding })
      : this.#instance;
    if (!('textEmbeddingModel' in embeddingProvider)) {
      throw new CopilotProviderNotSupported({
        provider: this.type,
        kind: 'embedding',
      });
    ...
    try {
      ...
      const modelInstance = embeddingProvider.textEmbeddingModel(model.id);
