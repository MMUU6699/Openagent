...
export class OAuthController {
  ...
  @Get('/providers')
  providers() {
    return { providers: this.providerFactory.providers.filter(name => name === 'google' || name === 'github') };
  }
  ...
  async preflight(
    ...
    if ((!client || client === 'web') && redirectUri &&
      (!/^\/(?!\/)/.test(redirectUri) || redirectUri.includes('\\'))) {
  ...
  async callback(
    ...
    let tokens: Tokens;
    // DEL succeeds for only one callback, preventing state replay.
    if (!(await this.oauth.consumeOAuthState(stateStr))) throw new OauthStateExpired();
    try {
      tokens = await provider.getToken(code);
    } catch (err) {
      this.logger.warn(`OAuth token exchange failed for ${state.provider}`);
      throw err;
    }
