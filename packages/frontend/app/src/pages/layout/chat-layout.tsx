  ...
  return (
    <Link to={`/chats/${chat.sessionId}`}>
      <li className={cn(styles.listItem, isActive && styles.activeItem)}>
        ...
        <div className={styles.listItemLabel}>{chat.title ?? t("New Chat")}</div>
      </li>
    </Link>
  );
};
  ...
  return (
    <div className="size-full flex flex-col">
      ...
      <div className="flex flex-col gap-1 px-2">
        <UserInfo />
        <Link to="/settings" className="px-2 py-2 text-sm">{t('Settings')}</Link>
        <Cmdk className="my-2" />
        <Link to="/chats">
          <li
          ...
          >
            ...
            <div className={styles.hoverableLabel}>{t("New Chat")}</div>
          </li>
        </Link>
        <Link to="/library">
          <li
          ...
          >
            ...
            <div className={styles.hoverableLabel}>{t("Library")}</div>
          </li>
        </Link>
      </div>
      ...
      <ScrollableContainer className="px-2 flex-1 h-0">
        ...
        <section className="my-2">
          {initialized ? null : loading ? (
            <div className="px-2 text-text-secondary text-sm flex items-center gap-3">
              <Loading className="text-xl" />{t("Loading history...")}</div>
          ) : null}
          {chats.length > 0 ? (
            <h3 className={cn(styles.sectionTitle, 'mb-1 mt-4', 'px-2')}>{t("Recent")}</h3>
          ) : null}
          <AnimatePresence>
        ...
        {collectedItems.length === 0 ? null : (
          <h3 className={cn(styles.sectionTitle, 'mb-2', 'pt-2 px-2', 'mt-2')}>{t("Favorites")}</h3>
        )}
        <AnimatePresence>
          <ul className="flex flex-col gap-1">
