...
type Step = 'methodSelect' | 'password' | 'register' | 'magic';
...
const _OAuthButton: React.FC<OAuthButtonProps> = ({ provider, redirectUrl }) => {
...
export const SignInPage: React.FC<{ initialStep?: Step }> = ({
  initialStep = 'methodSelect',
}) => {
  const navigate = useNavigate();
  const [searchParams] = useSearchParams();

  const requestedRedirect = searchParams.get('redirect') || '/chats';
  const redirectUrl =
    /^\/(?!\/)/.test(requestedRedirect) &&
    !requestedRedirect.includes('\\') &&
    !/^\/(sign-in|sign-up|onboarding)([/?#]|$)/.test(requestedRedirect)
      ? requestedRedirect
      : '/chats';
  ...
  }, [sessionChecked, isLoading, navigate, redirectUrl, user]);
      ...
      <div className={styles.wrapper}>
        {step === 'methodSelect' && (
          <>
            ...
            <div className="mt-4 text-center">
              <Link to={`/sign-up?redirect=${encodeURIComponent(redirectUrl)}`}>
        ...
        {step === 'password' && (
          <>
            ...
            {/* Email codes require an SMTP service, not configured here.
            <button
              onClick={() => {
                void (async () => {
                  await sendMagicLink(email, { redirectUrl });
        ...
        {step === 'register' && (
          ...
          >
            ...
            <div className="mt-4 text-center">
              <Link onClick={() => { clearError(); setStep('methodSelect'); }} to={`/sign-in?redirect=${encodeURIComponent(redirectUrl)}`}>
