...
const Empty = ({ active }: { active: 'chats' | 'docs' | 'files' | 'all' }) => {
  return (
    <div className="size-full flex flex-col justify-center items-center">
      ...
      />
      <span className="text-[15px] leading-[24px] color-black font-medium">{t("There are no contents here")}</span>
      <span
        style={{
          color: cssVarV2.text.secondary,
        }}
        className="text-sm leading-[22px]"
      >{t("You can generate content by creating new chats")}</span>

      <Link to="/chats" className="mt-4">
        <Button>{t("New Chat")}</Button>
      </Link>
    </div>
  );
};
...
const DateGroupHeader: MasonryGroup['Component'] = ({ groupId, itemCount }) => {
  return (
    <div className={cn('bg-white', styles.dateGroupHeader)}>
      {t(({today:'Today',yesterday:'Yesterday',thisWeek:'This Week',thisMonth:'This Month',older:'Older'} as Record<string,string>)[groupId] ?? groupId)}
...
const ChatListItem: MasonryItem['Component'] = ({ itemId }) => {
  ...
  const deleteSession = useCallback(async () => {
    ...
    try {
      ...
      toast(`${chat?.title ?? t("New Chat")} deleted`);
      refresh();
    } finally {
      setDeleting(false);
    }
  ...
  return (
    <Link to={`/chats/${itemId}`} className={deleting ? 'opacity-50' : ''}>
      <div className={styles.listItem}>
        ...
        <div className={styles.listItemTitle}>{chat?.title ?? t("New Chat")}</div>
        <div
    ...
    <div className="flex-1 panel overflow-hidden h-full flex flex-col">
      <header className="h-15 border-b-[0.5px] px-4 flex items-center gap-4">
        <AutoSidebarPadding className="transition-all h-full flex items-center">
          <span className={styles.library}>{t("Library")}</span>
        </AutoSidebarPadding>
