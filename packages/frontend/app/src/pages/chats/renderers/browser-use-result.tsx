    ...
    <GenericToolResult
      ...
      title={
        ...
        currentStatus === BROWSER_USE_TASK_STATUS.PAUSED ? (
          <span className="text-sm text-gray-600">{t("The browser task has been completed. Below are the steps and results.")}</span>
        ) : (
          <span className="text-sm text-gray-600">{t("The browser task is running. Below are the steps and results.")}</span>
        )
      }
    >
      <div className="max-h-150 overflow-y-auto">
        ...
        {stepsInfo &&
          ...
          finalMarkdown === null &&
          displayImage === null && (
            <div className="p-3 text-sm text-gray-500 text-center">{t("No detailed content available.")}</div>
          )}
      </div>
    </GenericToolResult>
  );
