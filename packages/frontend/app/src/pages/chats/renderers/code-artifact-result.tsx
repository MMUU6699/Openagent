...
export const CodeArtifactResult = ({
  ...
  const [view, setView] = useState<string>(t('Code'));
  ...
  return (
    ...
    >
      <div className={cn('relative', view === t("Preview") && 'h-150')}>
        <HtmlPreviewer
          code={html}
          className={cn(
            'size-full min-h-150 absolute',
            view === t("Code") ? 'opacity-0 pointer-events-none' : ''
          )}
        />
        {view === t("Code") ? (
          <div
