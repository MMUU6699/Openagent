  ...
  return (
    <div className="h-15 border-b-[0.5px] px-4 flex items-center justify-between gap-4">
      ...
      <div className="flex items-center gap-2">
        <div className="flex items-center gap-2">
          <button className="bg-black text-white px-4 py-1 h-8 rounded-md text-sm font-medium cursor-pointer">{t("Sign In")}</button>
        </div>
      </div>
    </div>
  );
    ...
    <div className="absolute inset-0 flex z-5">
      ...
      <div className="relative flex flex-col items-center justify-center self-end w-full h-[calc(100%-160px)] text-gray-500">
        <div className="text-sm w-[320px] text-center gap-4 flex flex-col">
          <span>{t("You are viewing a complete OpenAgent task. Playback will begin automatically in")}<span className="font-bold text-gray-600 ml-1">
              {countdown}{t("seconds")}</span>
          </span>

          <button
            className="bg-black text-white px-6 rounded-md text-sm h-10 font-medium cursor-pointer hover:bg-black self-center"
            onClick={onStart}
          >{t("Start playback")}</button>
        </div>
      </div>
    </div>
  );
    ...
    <OpenDocProvider value={{ openDoc, closeDoc }}>
      <div className="overflow-hidden h-full flex gap-2">
        <div className="flex-1 bg-white border rounded-[8px]  overflow-auto h-full">
          {id && sessionStore ? (
            <ChatPlaybackSession
              ...
              headerContent={
                <PlaybackHeader
                  title={sessionStore.getState().meta?.title ?? t("New Chat")}
                />
              }
        ...
        {!showCountdown && !finished && !skipped && (
          <Banner position="bottom">
            <span className="font-medium flex items-center gap-2">
              <img src="/logo.svg" alt="OpenAgent" className="size-6" />{t("OpenAgent is replaying task...")}</span>
            <button
              className="bg-black text-white px-4 py-1 rounded-md text-sm h-10 font-medium cursor-pointer"
              onClick={skipToEnd}
            >{t("Skip to results")}</button>
          </Banner>
        )}
        ...
        {(finished || skipped) && !showCountdown && (
          <Banner position="bottom">
            <span className="font-medium flex items-center gap-2">
              <img src="/logo.svg" alt="OpenAgent" className="size-6" />{t("OpenAgent replay completed")}</span>
            <button
              className="bg-black text-white px-4 py-1 rounded-md text-sm h-10 font-medium cursor-pointer"
              onClick={replay}
            >{t("Watch again")}</button>
          </Banner>
        )}
      </div>
    </OpenDocProvider>
