    ...
    >
      ...
      <div className="max-w-[800px] mx-auto px-4 py-3 flex items-center justify-between gap-4">
        <div className="text-sm font-medium">
          {totalTodos} {t('Todo Item')}
        </div>
        <div className="flex-1" />
        {/* Simple counts display */}
        <div className="flex items-center gap-3 text-xs text-gray-600">
          {groupedCounts.inProgress > 0 && (
            <span>{groupedCounts.inProgress}{t("In&nbsp;Progress")}</span>
          )}
          {groupedCounts.todo > 0 && <span>{groupedCounts.todo}{t("Todo")}</span>}
          {groupedCounts.done > 0 && <span>{groupedCounts.done}{t("Done")}</span>}
        </div>
