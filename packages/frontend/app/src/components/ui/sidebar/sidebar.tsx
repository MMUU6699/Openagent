...
export default function AppSidebar({
  ...
  const onDragMove = useCallback((clientX: number) => {
    const delta = (clientX - initialClientXRef.current) * (document.documentElement.dir === 'rtl' ? -1 : 1);
  ...
  const onDragEnd = useCallback(() => {
    const delta = (prevClientXRef.current - initialClientXRef.current) * (document.documentElement.dir === 'rtl' ? -1 : 1);
    const newWidth = Math.max(
      SIDEBAR_WIDTH_MIN,
      Math.min(SIDEBAR_WIDTH_MAX, initailWidthRef.current + delta)
    );
