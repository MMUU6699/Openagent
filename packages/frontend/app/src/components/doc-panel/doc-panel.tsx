...
export function DocPanelById({
    ...
    const loadDocument = async () => {
      try {
        ...
        if (store) {
          ...
          setState({
            status: 'error',
            error: t("Failed to create document"),
          });
        }
      } catch (err) {
        ...
        setState({
          status: 'error',
          error: t("Failed to load document"),
        });
      }
    };
  ...
  if (state.status === 'loading') {
    return (
      <div className="h-full flex items-center justify-center">
        <div className="text-gray-500">{t("Loading document...")}</div>
      </div>
    );
  }
  ...
  if (!state.doc) {
    return (
      <div className="h-full flex items-center justify-center">
        <div className="text-gray-500">{t("No document available")}</div>
      </div>
    );
  }
      ...
      <div className="flex items-center justify-between px-4 py-3 border-b-[0.5px] h-15">
        ...
        <div className="flex items-center gap-3">
          ...
          <IconButton
            ...
            title={t("Enter presentation mode")}
          />
          <IconButton
            size="24"
            icon={<CloseIcon />}
            onClick={onClose}
            title={t("Close document")}
          />
        </div>
      </div>
