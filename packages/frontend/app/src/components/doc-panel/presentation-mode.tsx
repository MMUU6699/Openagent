  ...
  useEffect(() => {
    ...
    const createNoteDoc = () => {
      try {
        ...
        if (!blockSnapshot) {
          console.error(t("Failed to create snapshot from note"));
          return;
        }
...
export function PresentationMode({ doc, onClose }: PresentationModeProps) {
    ...
    return (
      ...
      >
        <div className="text-center">
          <h2 className="text-2xl mb-4">{t("No content to present")}</h2>
          <p className="text-gray-600 mb-6">{t("No note blocks found in the document for presentation")}</p>
          <button
            onClick={exitPresentation}
            className="px-6 py-2 bg-blue-600 text-white rounded hover:bg-blue-700"
          >{t("Close Presentation")}</button>
        </div>
      </div>
    );
  }
  ...
  return (
    ...
    >
      ...
      >
        <div>{t("← → Navigate | ESC Exit | Space Next Page")}</div>
      </div>
    </div>
  );
}
